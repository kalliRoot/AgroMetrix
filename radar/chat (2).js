...
class ChatManager {
  ...
  getChatId(uid1, uid2) {
    // Formato simples sem data — igual ao acceptReq no radar.html
  ...
  renderMessage(m) {
    const isMe = m.uid === this.currentUser?.uid || m.from === this.currentUser?.uid;
    let time = '';
  ...
  async send(text) {
    ...
    try {
      const msgData = {
        ...
        from: this.currentUser.uid,   // compatibilidade com loadChatHistory do HTML
        name: this.getName(),
        fromName: this.getName(),
        text: text.trim(),
        ts: new Date().toISOString(), // compatibilidade com loadChatHistory do HTML
      ...
      if (this.currentTargetUid) {
        // 1. Notificação para o DESTINATÁRIO (dispara alerta)
        ...
        // 2. Notificação ESPELHO para o REMETENTE (aparece instantaneamente na sua lista)
        await addDoc(collection(this.db, 'notifications'), {
          ...
          createdAt: serverTimestamp(),
          createdAtMs: Date.now(),
          read: true, // Já lida, pois foi você quem mandou (evita tocar som pra você mesmo)
        });
      }
  ...
  async acceptRequest(targetUid, targetName, requestMessage) {
    ...
    try {
      ...
      // 2. Notificação ESPELHO para quem ACEITOU (aparece instantaneamente na sua lista)
      await addDoc(collection(this.db, 'notifications'), {
        to: this.currentUser.uid,
        from: targetUid,
